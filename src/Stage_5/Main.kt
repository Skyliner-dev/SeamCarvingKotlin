...
fun main(args:Array<String>) {
    ...
    DynamicSeam(myImage, transpose(energiesList.dropLast(1).toMutableList())).vSeam()
//    GreedySeam(myImage,energiesList.dropLast(1)).carving()
    saveImage(myImage, File(args[3].substringAfter("-out")))
}
fun transpose(e:MutableList<MutableList<BigDecimal>>): List<MutableList<BigDecimal>> {
    val res = mutableListOf(mutableListOf<BigDecimal>())
    for (i in 0 until e.first().size) {
...
class DynamicSeam(
    ...
    fun vSeam() {
        ...
        codeRed.forEach {
            image.setRGB(it.first,it.second, red)
        }
